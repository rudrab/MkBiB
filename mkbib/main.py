...
class Window(Gtk.ApplicationWindow):
  def __init__(self, application, giofile=None):
    ...
    EditButton = Gtk.MenuButton(None,
                                image=Gtk.Image(icon_name="list-add-symbolic"))
    ...
    self.all_fields["Author"].set_tooltip_text(
        "Search with Google or CrossRef")
    self.all_fields["Year"].set_tooltip_text(
        "Refine Author search with Google")
    ...
    apis = [
        "Search", "Search Google", "Search Crossref", "Search DOI",
        "Search by Title", "From PDF"
    ]
    for api in apis:
  ...
  def file_open_clicked(self, name, action):
    self.Dialog.FileChooser(
        ["Open Existing BiBTeX File", "BiBTeX File", "*.bib", True],
  ...
  def file_save_clicked(self, name, action):
    ...
    try:
      ...
      self.Messages.on_error_clicked("No file to save",
                                     "Try to 'save as' a new file")

  def file_save_as_clicked(self, name, action):
    self.Dialog.FileChooser(
        ["Save as an existing file", "BiBTeX File", "*.bib", True],
        Gtk.FileChooserAction.SAVE, Gtk.STOCK_SAVE)
    if self.Dialog.response == Gtk.ResponseType.OK:
      filename = self.Dialog.dialog.get_filename()
      print(filename)
  ...
  def activate_button(self, widget):
    if (self.KeyEntry.get_text_length() > 0
        and len(self.get_combo_data(self.name)) > 0):
  ...
  def activate_scholar(self, widget):
    if ((len(self.all_fields["Author"].get_text()) > 0)
        or (len(self.all_fields["DOI"].get_text()) > 0)
        or (len(self.all_fields["Title"].get_text()) > 0)
        or (len(self.all_fields["File"].get_text()) > 0)):
  ...
  def search_gschol(self, combo):
    ...
    if api_selected == 1:
      ...
      self.Datas.search_gs(datatup[1], datatup[3], datatup[2])
    # Search CrossRef
    elif api_selected == 2:
      neworder = [3, 0, 2, 1]
      fields = [Mkbib.fields[i] for i in neworder]
