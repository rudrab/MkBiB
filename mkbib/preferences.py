...
class file_manager():
  ...
  def move_file(self, source, destin):
    # print(subdir)
    # print(source)
    destin = destin[2].replace(" ",
                               "_") + "_" + destin[3].split()[0] + "_" + str(
                                   destin[5])  #+os.path.splitext(source)[1]
    destin = re.sub('[^A-Za-z0-9_-]+', '',
                    destin) + os.path.splitext(source)[1]
  ...
  def preferences(self, err1, err2):
    ...
    menuicon = Gtk.Image.new_from_icon_name("preferences-desktop-symbolic",
                                            Gtk.IconSize.MENU)
    ...
    self.user_dir.set_image(image=Gtk.Image(icon_name="folder-open-symbolic"))
    self.user_dir.set_image_position(1)
    self.user_dir.connect("clicked", self.set_folder_clicked)
    self.setting = Gio.Settings.new("org.example.mkbib")
    self.setting.bind("base-folder", self.user_dir, "label",
  ...
  def set_folder_clicked(self, name):
    self.Dialog.FileChooser(["Choose Folder", "", "", False],
    ...
    if self.Dialog.response == Gtk.ResponseType.OK:
      ...
      self.Messages.on_info_clicked("Setting Updated",
                                    "Restart MkBiB to enable changes.")
